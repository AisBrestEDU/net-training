...
namespace Reflection.Tasks
{
	public class CodeGeneration
	{
		...
		public static Func<T[], T[], T> GetVectorMultiplyFunction<T>() where T : struct
		{
			...
			ParameterExpression firstArray = Expression.Variable(typeof(T[]), "firstArray");
			ParameterExpression secondArray = Expression.Variable(typeof(T[]), "secondArray");
			...
			ParameterExpression[] variables = new[] { result, index };
			var a = Expression.Assign(index, Expression.ArrayLength(firstArray));
			var b = Expression.Assign(result, Expression.Constant(default(T)));
				...
				Expression.Loop(
					Expression.IfThenElse(
						...
						Expression.AddAssign(result,
							Expression.Multiply(
								Expression.ArrayAccess(firstArray, Expression.PreDecrementAssign(index)),
								Expression.ArrayAccess(secondArray, index))),
			...
			return Expression.Lambda<Func<T[], T[], T>>(block, firstArray, secondArray).Compile();
		}
